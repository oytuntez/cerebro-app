...
delete from storage.objects where bucket_id = 'app';
delete from storage.buckets where id = 'app';
...
insert into storage.buckets (id, name, public) values ('app', 'app', true);

create policy "Public access for all users" on storage.objects
  for select to authenticated, anon using (bucket_id = 'app');
create policy "User can upload in their own folders" on storage.objects
  for insert to authenticated with check (bucket_id = 'app' and (storage.foldername(name))[1] = (select auth.uid()::text));
...
-- select create_new_user('oytuntez@gmail.com', '123456789');
-- select delete_user('oytuntez@gmail.com');

select assign_user_data();

select set_user_role('superadmin', null, 'oytuntez@gmail.com');
select set_user_plan('premium', null, 'oytuntez@gmail.com');
